...
import br.com.daysesoares.helpdesk.api.dto.Summary;
...
public class TicketController {
	...
	public ResponseEntity<Response<Ticket>> createOrUpdate(HttpServletRequest request, @RequestBody Ticket ticket, BindingResult result){
		...
		try {
			...
			ticket.setDateRegistration(new Date());
			ticket.setNumber(generateNumber());
			Ticket ticketPersistend = ticketService.createOrUpdate(ticket);
	...
	public ResponseEntity<Response<Ticket>> update(HttpServletRequest request, 
		...
		try {
			...
			ticket.setDateRegistration(ticketCurrent.getDateRegistration());
	...
	public ResponseEntity<Response<Page<Ticket>>> findAll(HttpServletRequest request, 
		...
		for (ProfileEnum profile : userRequest.getProfiles()) {
			if(profile == ProfileEnum.ROLE_TECHNICIAN){
				tickets = ticketService.listTicket(page, count);
			}else if(profile == ProfileEnum.ROLE_CUSTOMER){
	...
	public ResponseEntity<Response<Ticket>> changeStatus(
		...
		Response<Ticket> response = new Response<Ticket>();
		try{
			validadateChangeStatus(id, status, result);
			if(result.hasErrors()){
				result.getAllErrors().forEach(error -> response.getErrors().add(error.getDefaultMessage()));
				return ResponseEntity.badRequest().body(response);
			}
	...
	private void validadateChangeStatus(String id, String status, BindingResult result){
		...
		if(tickets != null){
			for(Iterator<Ticket> iterator = tickets.iterator(); iterator.hasNext();){
				...
				if(ticket.getStatus().equals(StatusEnum.Disapproved)){
					amountDisapproved = amountDisapproved++;
				}
